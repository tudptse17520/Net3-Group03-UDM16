...
public partial class GameBoardForm
{
    ...
    private readonly RoomLockButton _spectatorLock = new() { Name = "SpectatorLock", Enabled = false };
    ...
    private RoomOfferForm? _newGameDialog;
    private bool _isRoomOwner;
    private long _lastMoveNoticeAt;
    ...
    private void InitializeSocialUi()
    {
        ...
        {
            if (!_isRoomOwner || _isSpectator) return;
    ...
    private async Task SendSocialAsync(MessageType type, object payload)
    {
        ...
        {
            if (!IsDisposed) { _spectatorLock.Enabled = _isRoomOwner && !_isSpectator; ShowSocialNotice("Mất kết nối, vui lòng thử lại."); }
        }
    }
    private void LayoutSocialUi(float scale)
    {
        ...
        int lockSize = (int)(30 * scale);
        int codeWidth = Math.Min(pnlPlayer1.Width - lockSize - gap, TextRenderer.MeasureText(_roomCode.Text, _roomCode.Font).Width + 6);
        _roomCode.SetBounds(pnlPlayer1.Left + (pnlPlayer1.Width - codeWidth - lockSize - gap) / 2, _matchClock.Top, codeWidth, lockSize);
        _spectatorLock.SetBounds(_roomCode.Right + gap, _roomCode.Top, lockSize, lockSize);
        _spectatorNotice.SetBounds(pnlPlayer1.Left, _spectatorLock.Bottom + gap, pnlPlayer1.Width, (int)(44 * scale));
        _spectatorLock.Visible = true;
        if (_isSpectator)
        {
        ...
        _spectators.SetBounds(pnlWoodFrame.Left, btnExitMatch.Bottom + gap, pnlWoodFrame.Width, (int)(82 * scale));
        ...
        PositionNewGameOffer();
    }
    private void PositionNewGameOffer()
    {
        if (_newGameDialog is not { IsDisposed: false }) return;
        int gap = (int)(8 * DeviceDpi / 96f);
        int top = lblAppTitle.Bottom + gap;
        int height = Math.Min((int)(160 * DeviceDpi / 96f), pnlPlayer2.Top - top - gap);
        var point = PointToScreen(new Point(pnlPlayer2.Left, pnlPlayer2.Top - height - gap));
        _newGameDialog.Bounds = new Rectangle(point, new Size(pnlPlayer2.Width, height));
    }
    private void ShowMoveNotice(string text)
    {
        if (_isSpectator || Environment.TickCount64 - _lastMoveNoticeAt < 1800) return;
        _lastMoveNoticeAt = Environment.TickCount64;
        var card = _mySymbol == 1 ? pnlPlayer1 : pnlPlayer2;
        ShowSocialNotice(text);
        _spectatorNotice.SetBounds(card.Left, card.Top - (int)(42 * DeviceDpi / 96f), card.Width, (int)(36 * DeviceDpi / 96f));
    }
    private void ShowSocialNotice(string text)
    {
        _spectatorNotice.SetBounds(pnlPlayer1.Left, _roomCode.Bottom + 8, pnlPlayer1.Width, (int)(44 * DeviceDpi / 96f));
    ...
    private string SpectatorResultText(int winner)
    {
        if (winner == 0) return "Ván đấu kết thúc hòa.";
        string name = winner == 1 ? lblPlayer1Name.Text : lblPlayer2Name.Text;
        return $"Chúc mừng {name} đã chiến thắng!";
    }
    private void ApplyRoomPresence(RoomDto? room)
    {
        ...
        _isRoomOwner = room.OwnerName == NetworkClient.Instance.CurrentNickname;
        _spectatorLock.Locked = _spectatorLocked;
        _spectatorLock.Enabled = _isRoomOwner && !_isSpectator;
        _sharedToolTip.SetToolTip(_spectatorLock, _isRoomOwner ? (_spectatorLocked ? "Mở phòng cho khán giả" : "Khóa phòng với khán giả mới") : "Chỉ chủ phòng được đổi khóa");
    ...
    private void ShowNewGameOffer(NewGameOfferDto offer)
    {
        ...
        SetWaiting(offer.RequesterName == NetworkClient.Instance.CurrentNickname ? "Đang chờ đối thủ phản hồi..." : "Đối thủ đề nghị ván mới", "rematch");
        ...
        _newGameDialog = new RoomOfferForm(offer.RequesterName);
