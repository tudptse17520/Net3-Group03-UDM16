        ...
        private void HandleIncomingMessage(NetworkMessage message)
        {
            switch (message.Type)
            {
                ...
                case MessageType.JoinSpectatorResponse:
                    if (message.Payload is JsonElement spectateRespElement)
                    {
                        var resp = spectateRespElement.Deserialize<JoinSpectatorResponse>(JsonOptions);
                        if (resp != null)
                        {
                            OnSpectatorJoined?.Invoke(resp);
                        }
                    }
                    break;
